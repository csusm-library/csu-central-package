...
/* begin PRIMO Angular stuffs */
	(function() {
		'use strict';

		var app = angular.module('viewCustom', ['angularLoad', 'sendSms','reportProblem','multipleAnalytics', 'externalSearch']);

		/**
		 * Enable Central Package Modules
		 */
		...
		app.constant('analyticsOptions', {
			...
			siteSource: 'matomo',
			siteId: '2',
			siteUrl: '//library.csudh.edu/analytics/',
			defaultTitle: 'CSUDH University Library OneSearch'
		});
		app.constant('reportProblem', {
			enabled: false,
			reportVendor: 'libanswers',
			instid: '911',
			quid: '2426',
			qlog: '0',
			source: '4',
			subject: 'ER Access Report',
			reportUrl: 'https://slips.calstate.edu/problem-test/'
		});
		...
		app.component('prmLogoAfter', {
			...
			template: '<div class="product-logo product-logo-local" layout="row" layout-align="start center" layout-fill id="banner"><a href="https://www.csudh.edu/library/"><img class="logo-image" alt="{{::(\'nui.header.LogoAlt\' | translate)}}" ng-src="{{$ctrl.getIconLink()}}"/></a></div>'
		});
		...
		app.component('prmNoSearchResultAfter', {
			...
			templateUrl: 'custom/01CALS_UDH/html/prmNoSearchResultAfter.html'
		});

		/**
		 * Custom dropdown and mobile navigation
		 */

		app.controller('prmTopbarAfterController', ['$rootScope', '$translate', function($rootScope, $translate) {
			$translate.onReady(function() {
				/* begin language selection dynamic generation */
				/* leaving this here in case language selection is added back, no need to reinvent the wheel */
				...
				//}
				//langsOptions += '</ul>';
				/* end language selection dynamic generation */

				/* begin user login check */
					//console.log($rootScope.$$childTail.$ctrl);
					//parent.postMessage($rootScope.$$childTail.$ctrl.storageutil.sessionStorage.primoExploreJwt, "http://libws02.campus.csudh.edu");
					...
					var signInOutButtonText = isLoggedIn ? ($translate.instant('eshelf.signout.title.link') + ' - ' + user.userName) : ($translate.instant('eshelf.signin.title') + ' - ' + user.userGroup);
					var myAccountButtonText = $translate.instant('nui.aria.menu.librarycard');
					var librarySearchButtonText = $translate.instant('mainmenu.label.library_search');
					var journalSearchButtonText = $translate.instant('nui.mainmenu.label.journalsearch');
				/* end user login check */
				
				/* begin mobile menu initialization */
				jQuery('#nav').meanmenu({
					...
					meanAdditionalListElements: [
						/* begin primo mobile submenu */
						{
							content: `
								<a class="preventSlide">OneSearch</a>
								<ul>
									<li>
										<a href="#" onclick="event.preventDefault(); $('prm-authentication button').click();">`+signInOutButtonText+`</a>
									</li>
									<li>
										<a href="#" onclick="event.preventDefault(); $('prm-library-card-menu button').click();" aria-label="`+myAccountButtonText+`">`+myAccountButtonText+`</a>
									</li>
									<li>
										<a href="http://csudh-primosb.hosted.exlibrisgroup.com/primo-explore/search?vid=01CALS_UDH" aria-label="`+librarySearchButtonText+`">`+librarySearchButtonText+`</a>
									</li>
									<li>
										<a href="http://csudh-primosb.hosted.exlibrisgroup.com/primo-explore/jsearch?vid=01CALS_UDH" aria-label="`+journalSearchButtonText+`">`+journalSearchButtonText+`</a>
									</li>
								</ul>`,
							position: 'start'
						}/*,
						{//don't delete this, this is for the home page, but #audiencenav doesn't exist in primo
							target: '#audiencenav',
							position: 'end'
						}
						/* end primo mobile submenu */
					],
					meanLogoTarget: '#logo img',
					meanLogoSrc: 'http://www.csudh.edu/Assets/CSUDH-Sites/Brand/docs/csudh_logo_White.png',
				...
				/* end mobile menu initialization */
			});
		}]);
		...
		app.component('prmTopbarAfter', {
			...
			templateUrl: 'custom/01CALS_UDH/html/prmTopbarAfterController.html'
		});
						...
						<div class="md-chips md-chips-wrap">
							<div ng-repeat="target in targets" aria-live="polite" class="md-chip animate-opacity-and-scale facet-element-marker-local4">
								<div class="md-chip-content layout-row" role="button" tabindex="0">
									<a class="facet-title" ng-href="{{ target.url + target.mapping(queries, filters) }}" target="_blank">
		...
		.factory('externalSearch', function () {
			return {
				...
				addExtSearch: function () {
					var found = false;
					this.prmFacetCtrl.facetService.results.filter(opt => opt.name === 'External Search' ? found = true : '')
					if(!found) this.prmFacetCtrl.facetService.results.splice(3, 0, {name: 'External Search', displayedType: 'exact', limitCount: 0, facetGroupCollapsed: false, values: undefined})
				}
			}
		})
		app.value('searchTargets', [
			{
				...
				"img": "https://www.oclc.org/etc/designs/oclc/oclc-annualreport-14/images/worldcat/wc-pinwheel.png",
...
/* end PRIMO Angular stuffs */


/* begin chat widget stuffs */
	var d=document.createElement('div');
	d.id='libchat_edf02a6683ec58d01c7583e2b7bce337';
	document.body.appendChild(d);
	var s=document.createElement('script');
	s.id='localScript';
	s.src='//v2.libanswers.com/load_chat.php?hash=edf02a6683ec58d01c7583e2b7bce337';
	document.body.appendChild(s);
/* end chat widget stuffs */
